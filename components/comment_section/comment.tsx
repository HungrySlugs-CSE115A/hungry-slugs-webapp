...
const Comment: React.FC<CommentProps> = ({
  ...
  const timePosted = new Date(comment.time_posted).toLocaleString(
    "en-US",
    options,
  );
  const isReplying =
    ...
    <div className="comment flex p-4">
      ...
      <div className="comment_right_part w-full">
        <div className="comment_content flex">
          <div className="comment_author mr-2 text-xl text-blue-500">
            {comment.username}
          </div>
          <div>{timePosted}</div>
        </div>
        {!isEditing && (
          <div className="comment_text text-base break-all max-w-full">
            {comment.body}
          </div>
        )}
        {isEditing && (
        ...
        <div className="comment_actions flex text-sm text-gray-700 cursor-pointer mt-2">
          {canReply && (
            <div
              className="comment_action mr-2 hover:underline"
              onClick={() =>
